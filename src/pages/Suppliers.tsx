...
import { Search, UserPlus, Phone, Send, ChevronRight, ArrowDownRight, ArrowUpRight, History, X, Loader2, Users, Plus } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { sendSMS } from '@/shared/lib/netgsm';
import { EmptyState } from '@/shared/ui/EmptyState';
import { supabase } from '@/shared/api/supabase';
import { useEntities, useAddEntity, useUpdateEntityBalance, type Entity } from '@/shared/hooks/useEntities';
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 -mt-2">
        ...
        <Button onClick={() => setIsAddOpen(true)} className="flex-shrink-0 w-full md:w-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card padding="md" className="bg-white border-gray-200 lg:col-span-1">
          <div className="flex justify-between items-center h-full">
            <div>
              <p className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-1">Piyasadaki Borcunuz</p>
              <p className="text-3xl font-bold text-red-600">{totalReceivables.toLocaleString('tr-TR', {minimumFractionDigits:2})} ₺</p>
            </div>
            <div className="p-4 bg-red-50 rounded-full flex-shrink-0">
        ...
        <Card padding="sm" className="bg-white border-gray-200 lg:col-span-2 overflow-hidden flex flex-col max-h-[160px]">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 px-2">Yaklaşan Ödemeler</h3>
          <div className="flex-1 overflow-y-auto px-2 space-y-2 scrollbar-hide">
            {upcomingPayments.length === 0 ? (
              <p className="text-sm text-gray-400 py-4 text-center">Planlanmış bir ödeme yok.</p>
            ) : (
              upcomingPayments.map(tx => {
                const isOverdue = new Date(tx.due_date) < new Date(new Date().setHours(0,0,0,0));
                return (
                  <div key={tx.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50 border border-gray-100 cursor-pointer hover:bg-red-50 transition-colors" onClick={() => openLedger(customers.find(c => c.id === tx.customer_id) as any)}>
                    <div>
                      <p className="text-sm font-bold text-gray-900">{tx.customers?.name}</p>
                      <p className={`text-xs font-medium ${isOverdue ? 'text-red-500' : 'text-gray-500'}`}>
                        Vade: {new Date(tx.due_date).toLocaleDateString('tr-TR')} {isOverdue && '(Gecikti!)'}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-gray-900">{Number(tx.amount).toLocaleString('tr-TR')} ₺</p>
                    </div>
                  </div>
                )
              })
      ...
      <BottomSheet isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Yeni Toptancı Ekle">
        <div className="space-y-4 pt-4 pb-8">
          ...
          <Button 
            ...
            disabled={!newName || addCustomerMutation.isPending}
